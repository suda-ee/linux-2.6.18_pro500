...
#ifdef CONFIG_OF
static int __devexit xgpio_of_probe(struct of_device *dev,
				    const struct of_device_id *match)
{
	...
	xgpio_inst = kzalloc(sizeof(struct xgpio_instance), GFP_KERNEL);
	if (!xgpio_inst) {
		dev_err(&dev->dev, "Couldn't allocate device private record\n");
		return -ENOMEM;
	}

	/* Map the control registers in */
	rc = of_address_to_resource(dev->node, 0, &r_mem);
	if ((rc) || (r_mem.end - r_mem.start + 1 < 8)) {
		dev_warn(&dev->dev,
			 " Couldn't get correct registers resource\n");
		retval = -EFAULT;
		goto failed1;
	}

	xgpio_inst->remap_size = r_mem.end - r_mem.start + 1;
	if (!request_mem_region(r_mem.start, xgpio_inst->remap_size,
				DRIVER_NAME)) {
		printk(KERN_ERR "Couldn't lock memory region at 0x%08lX\n",
		       r_mem.start);
		retval = -EBUSY;
	...
	v_addr = ioremap(r_mem.start, xgpio_inst->remap_size);
	if (!v_addr) {
		printk(KERN_ERR "Couldn't ioremap memory at 0x%08lX\n",
		       r_mem.start);
		retval = -EFAULT;
	...
	xgpio_inst->base_phys = r_mem.start;
	/* The 1st GPIO channel uses */
	id = of_get_property(dev->node, "device-id", NULL);
	if (!id) {
		dev_err(&dev->dev, " Could not get device id.\n");
		retval = -ENODEV;
		goto failed3;
	}
	xgpio_inst->device_id = xgpio_config.DeviceId = *id;
	dl = of_get_property(dev->node, "dual-channel", NULL);
	if (!dl) {
		dev_err(&dev->dev, "Can not get \"dual-channel\" property.\n");
		retval = -ENODEV;
		goto failed3;
	}
	xgpio_config.IsDual = *dl;

	/* Tell the Xilinx code to bring this GPIO interface up. */
	if (XGpio_CfgInitialize(&xgpio_inst->gpio, &xgpio_config,
				(u32) v_addr) != XST_SUCCESS) {
		dev_err(&dev->dev, " Could not initialize instance %d.\n",
			xgpio_inst->device_id);
		retval = -ENODEV;
		goto failed3;
	}
	...
	iounmap((void *)(xgpio_config.BaseAddress));
      ...
      failed2:
	release_mem_region(r_mem.start, xgpio_inst->remap_size);
...
static int __devexit xgpio_of_remove(struct of_device *dev)
{
	return xgpio_remove(&dev->dev);
}
...
static int __init xgpio_init(void)
{
	 ...
	 */
	int status = driver_register(&xgpio_driver);
#ifdef CONFIG_OF
	status |= of_register_platform_driver(&xgpio_of_driver);
#endif
	return status;
}
...
#ifdef CONFIG_OF
	of_unregister_platform_driver(&xgpio_of_driver);
#endif
}
