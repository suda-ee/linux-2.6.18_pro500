 ...
 *         source@mvista.com
 *
 * 2007 (c) MontaVista, Software, Inc.  This file is licensed under the terms
...
static void emac_poll_controller(struct net_device *dev)
{
	unsigned long flags;
	spin_lock_irqsave(&reset_lock, flags);
	xenet_interrupt(dev->irq, dev, NULL);
	spin_unlock_irqrestore(&reset_lock, flags);
}
#endif
...
static int xenet_SgSendDre(struct sk_buff *skb, struct net_device *dev)
{
	...
	for (i = 0; i < (num_frag + 1); ++i) {
		...
		if (i == 0) {
			...
			/*
			 * The kernel does not send frames down with CHECKSUM_HW
			...
			if ((XEmac_mIsTxHwCsum(&lp->Emac))
				...
				if (skb->ip_summed == CHECKSUM_HW) {
					 ...
					 */
					csum_insert_offset =
					    IpHeaderLength + 16 + ENET_HDR_SIZ;
...
static void SgRecvHandlerBH(unsigned long p)
{
	...
	while (1) {
		...
		while (NumBds != 0) {
			...
			skb_vaddr =
			    (dma_addr_t) XBufDescriptor_GetDestAddress(curbd);
			 ...
			 *     skb_put(skb, len);
			 *   However, doing this cause the driver not to work anymore.
			 *   If you want to fix this and put in the skb_pu() again, you
			 *   need to delete the
			 *     skb->len -= 4;
			 *   line below.
			 * wgr 09/14/2006
			...
			if ((len > 76 /*64 */ )
				...
				if (((IpDataLen & 0x0003) == 2)
					 ...
					 */
					EmacFCSPtr[0] =
					    (*(u8 *) (skb->mac.raw + len - 4));
					EmacFCSPtr[1] =
					    (*(u8 *) (skb->mac.raw + len - 3));
					EmacFCSPtr[2] =
					    (*(u8 *) (skb->mac.raw + len - 2));
					EmacFCSPtr[3] =
					    (*(u8 *) (skb->mac.raw + len - 1));
				...
				PhCSum =
				    AddCsumRxPseudoHeader(skb, HwCSum,
							  IpDataLen, ProtoTTL);
...
/*set MAX_MULTICAST to actual value when hw filtering will be supported*/
#define  MAX_MULTICAST 0
		...
		* this skeleton code should be fleshed out.
		*/
...
static int __devexit xenet_remove(struct of_device *of_dev)
{
...
static int __devinit xenet_get_config(struct device_node *dev_node,
				      XEmac_Config * Config)
{
	u32 val;;

	/* Read config from device-tree */
	if (emac_read_uint_prop(dev_node, "device-id", &val, 1))
		return -ENXIO;

	Config->DeviceId = (u16) val;

	if (emac_read_uint_prop(dev_node, "dma-mode", &val, 1))
		val = 1;

	Config->IpIfDmaConfig = (u8) val;

	if (emac_read_uint_prop(dev_node, "has-mii", &Config->HasMii, 1))
		return -ENXIO;
	if (emac_read_uint_prop(dev_node, "has-cam", &Config->HasCam, 1))
		return -ENXIO;
	if (emac_read_uint_prop(dev_node, "has-jumbo", &Config->HasJumbo, 1))
		return -ENXIO;
	if (emac_read_uint_prop(dev_node, "tx-dre", &Config->TxDre, 1))
		return -ENXIO;
	if (emac_read_uint_prop(dev_node, "rx-dre", &Config->RxDre, 1))
		return -ENXIO;
	if (emac_read_uint_prop(dev_node, "tx-hw-csum", &Config->TxHwCsum, 1))
		return -ENXIO;
	if (emac_read_uint_prop(dev_node, "rx-hw-csum", &Config->RxHwCsum, 1))
		return -ENXIO;

	return 0;
}

static int xenet_probe(struct of_device *of_dev,
		       const struct of_device_id *match)
{
	...
	rc = xenet_get_config(dev_node, &Config);
	if (rc)
		goto error;
	...
	if (ndev->irq == NO_IRQ) {
		printk(KERN_ERR "XEmac %d: irq not found.\n", Config.DeviceId);
		rc = -ENODEV;
		goto error;
	}

	if (of_address_to_resource(dev_node, 0, &r_mem)) {
		printk(KERN_ERR "XEmac: memory not found.\n");
		rc = -ENODEV;
		goto error;
	}
	...
	for (phy_addr = 0; phy_addr < 31; phy_addr++) {
		...
		for (repeat = 0; repeat < 4; repeat++) {
			...
			Result =
			    XEmac_PhyRead(&lp->Emac, phy_addr, MII_PHYSID1,
					  &check_reg);
		...
		if (4 == repeat) {
			 ...
			 */
			Result =
			    XEmac_PhyRead(&lp->Emac, phy_addr, MII_PHYSID2,
					  &rev);
			if (XST_SUCCESS != Result) {
				printk
				    ("XEmac: Error reading PHY revision for PHY at address %d! "
				     "THIS SHOULD NOT HAPPEN! Ignoring this PHY.\n",
				     phy_addr);
				continue;
			}
			 ...
			 */
			printk
			    ("XEmac: Detected PHY at address %d, ManufID 0x%04x, Rev. 0x%04x.\n",
			     phy_addr, reg, rev);
...
static struct of_device_id xilinx_emac_match[] = {
	{
	 .type = "network",
...
static struct of_platform_driver xenet_driver = {
	.name = DRIVER_NAME,
	.match_table = xilinx_emac_match,
	.probe = xenet_probe,
	.remove = xenet_remove
};
...
static int __init xenet_init(void)
{
	 ...
	 */
	return of_register_platform_driver(&xenet_driver);
}

static void __exit xenet_cleanup(void)
{
	of_unregister_platform_driver(&xenet_driver);
}
