			 ...
			 unsigned ByteCount, u8 Option)
{
	...
	if (Option == XIIC_STOP) {
		 ...
		 */
		while (1) {
			if (XIIF_V123B_READ_IISR(BaseAddress) &
			    XIIC_INTR_BNB_MASK) {
				break;
			}
		}
		   ...
		   u8 * BufferPtr, unsigned ByteCount, u8 Option)
{
	...
	if ((ControlReg & XIIC_CR_REPEATED_START_MASK) == 0) {
		...
		if ((ControlReg & XIIC_CR_MSMS_MASK) != 0) {
			...
			while ((StatusReg & XIIC_SR_BUS_BUSY_MASK) != 0) {
				StatusReg =
				    XIo_In8(BaseAddress + XIIC_SR_REG_OFFSET);
			}
		}
	...
	while (ByteCount > 0) {
		...
		if (ByteCount > 1) {
			...
			if (Option == XIIC_REPEATED_START) {
				...
				while (1) {
					IntrStatus =
					    XIIF_V123B_READ_IISR(BaseAddress);
					if (IntrStatus &
					    XIIC_INTR_TX_EMPTY_MASK) {
						/* RSTA bit should be set only when the FIFO is completely Empty.
						 */
		      ...
		      u8 ByteCount, u8 Option)
{
	 ...
	 */
	XIic_mDynSend7BitAddress(BaseAddress, Address, XIIC_WRITE_OPERATION);
