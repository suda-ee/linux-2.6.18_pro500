...
RCS_FIND_IGNORE := \( -name SCCS -o -name BitKeeper -o -name .svn -o -name CVS -o -name "$(QUILT_PATCHES)" -o -name "$(QUILT_PC)" -o -name .hg -o -name .git \) -prune -o
export RCS_TAR_IGNORE := --exclude SCCS --exclude BitKeeper --exclude .svn --exclude CVS --exclude "$(QUILT_PATCHES)" --exclude "$(QUILT_PC)" --exclude .hg --exclude .git
...
include/config/auto.conf:
	$(Q)test -f include/linux/autoconf.h -a -f $@ || (		\
	echo;								\
...
quiet_cmd_vmlinux_version = GEN     .version
	...
	if [ ! -r .version ]; then			\
	  ...
	  expr $(cat .old_version) + 1 >.version;	\
	fi;						\
	$(MAKE) $(build)=init
