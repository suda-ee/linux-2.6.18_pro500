/* $Id: pro_powerpc_xilinx_emac.patch,v 1.1.2.1 2008/03/15 09:18:39 pkiryukhin Exp $ */
/******************************************************************************
*
...
XStatus XEmac_FifoSend(XEmac * InstancePtr, u8 * BufPtr, u32 ByteCount)
{
	...
	if (InstancePtr->Poll_FIFO_TX_Done)
	{
		u32 IntrStatus,XmitStatus;

		do { IntrStatus =
				XIIF_V123B_READ_IISR(InstancePtr->BaseAddress);
		}
		while (!(IntrStatus & XEM_EIR_XMIT_DONE_MASK));
