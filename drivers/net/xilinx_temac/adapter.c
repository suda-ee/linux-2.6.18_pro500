...
int renegotiate_speed(struct net_device *dev, int speed, DUPLEX duplex)
{
	...
	status |= XTemac_PhyRead(&lp->Emac, lp->gmii_addr, MII_BMSR, &phy_reg1);
	status |=
	    XTemac_PhyWrite(&lp->Emac, lp->gmii_addr, MII_ADVERTISE, phy_reg4);
	status |=
	...
	while (retries--) {
		...
		status =
		    XTemac_PhyWrite(&lp->Emac, lp->gmii_addr, MII_BMCR,
				    phy_reg0);
		if (status != XST_SUCCESS) {
			printk(KERN_ERR
		...
		while (wait_count--) {
			...
			mdelay(500);
			status =
			    XTemac_PhyRead(&lp->Emac, lp->gmii_addr, MII_BMSR,
					   &phy_reg1);
			status |=
			    XTemac_PhyRead(&lp->Emac, lp->gmii_addr, MII_BMSR,
					   &phy_reg1);
...
static void reset(struct net_device *dev, u32 line_num)
{
	...
	if (XTemac_mIsSgDma(&lp->Emac)) {
		 ...
		 */
		(XStatus) XTemac_IntrSgCoalGet(&lp->Emac, XTE_RECV,
					       &RxThreshold, &RxWaitBound);
		(XStatus) XTemac_IntrSgCoalGet(&lp->Emac, XTE_SEND,
...
#ifdef MARVELL_88E1111_PHY
	xs = XTemac_PhyRead(&lp->Emac, lp->gmii_addr, MII_PHYSID1, &reg);
	if (xs != XST_SUCCESS) {
		printk(KERN_ERR
...
#define MARVELL_PHYSID_1 0x0141
/* link detection workaround: we can read MII regiters */
/* correctly only if link is down		      */
	if (reg == MARVELL_PHYSID_1)
	    *linkup = 0;
	else
	    *linkup = 1;
#else
...
xenet_SgSend_internal(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (total_frags < XTE_SEND_BD_CNT) {
		result =
		    XTemac_SgAlloc(&lp->Emac, XTE_SEND, total_frags, &bd_ptr);
	...
	if (skb->ip_summed == CHECKSUM_HW) {
		...
		XTemac_mSgSendBdCsumSetup(bd_ptr, skb->h.raw - skb->data,
	...
	/* Enqueue to HW */
	result =
	    XTemac_SgCommit(&lp->Emac, XTE_SEND, total_frags, first_bd_ptr);
	if (result != XST_SUCCESS) {
		netif_stop_queue(dev);	/* stop send queue */
		dev_kfree_skb(skb);
	...
	while (1) {
		...
		while ((bd_processed =
			...
			do {
				...
				/* get ptr to skb */
				skb =
				    (struct sk_buff *)XDmaBdV3_mGetId(BdCurPtr);
				if (skb)
					dev_kfree_skb(skb);
		...
		if ((bd_processed =
			...
			do {
				...
				if (((lp->
					 ...
					 */
					csum =
					    XTemac_mSgRecvBdCsumGet(BdCurPtr);
...
void disp_bd_ring(XDmaV3_BdRing bd_ring)
{
	...
	printk
	    ("Idx     Addr    DMASR     LSBA      BDA   Length     USR0     USR1     USR5       ID\n");
	printk
	    ("--- -------- -------- -------- -------- -------- -------- -------- -------- --------\n");
...
static int xtenet_remove(struct of_device *of_dev)
{
...
static int __devinit temac_read_uint_prop(struct device_node *np,
...
static int __devinit xtenet_get_config(struct device_node *dev_node,
				       XTemac_Config * Config)
{
	u32 val;

	/* Read config from device-tree */
	if (temac_read_uint_prop(dev_node, "device-id", &val, 1))
		return -ENXIO;

	Config->DeviceId = (u16) val;

	if (temac_read_uint_prop(dev_node, "rx-csum", &val, 1))
		val = 0;
	Config->RxCsum = (u8) val;
	if (Config->RxCsum) {
		if (temac_read_uint_prop(dev_node, "tx-dre", &val, 1))
			return -ENXIO;
		Config->TxDre = (u8) val;
		if (temac_read_uint_prop(dev_node, "rx-dre", &val, 1))
			return -ENXIO;
		Config->RxDre = (u8) val;
		if (temac_read_uint_prop(dev_node, "tx-csum", &val, 1))
			return -ENXIO;
		Config->TxCsum = (u8) val;
		if (temac_read_uint_prop
		    (dev_node, "rx-fifo-depth", &Config->RxPktFifoDepth, 1))
			return -ENXIO;
		if (temac_read_uint_prop
		    (dev_node, "tx-fifo-depth", &Config->TxPktFifoDepth, 1))
			return -ENXIO;
		if (temac_read_uint_prop(dev_node, "phy-type", &val, 1))
			return -ENXIO;
		Config->PhyType = (u8) val;
	} else {
		if (temac_read_uint_prop
		    (dev_node, "ipif-rdfifo-depth", &Config->RxPktFifoDepth, 1))
			return -ENXIO;
		if (temac_read_uint_prop
		    (dev_node, "ipif-wrfifo-depth", &Config->TxPktFifoDepth, 1))
			return -ENXIO;
	}

	if (temac_read_uint_prop(dev_node, "dma-mode", &val, 1))
		val = 1;

	Config->IpIfDmaConfig = (u8) val;

	if (temac_read_uint_prop(dev_node, "mac-fifo-depth", &val, 1))
		return -ENXIO;
	Config->MacFifoDepth = (u16) val;

	return 0;
}
#define DEBUG_TEMAC_MULTICAST
...
static int xtenet_probe(struct of_device *of_dev,
			const struct of_device_id *match)
{
	...
	rc = xtenet_get_config(dev_node, &Config);
	if (rc)
		goto error;
	...
	dev_set_drvdata(&of_dev->dev, ndev);

	/* Get iospace and an irq for the device */
	ndev->irq = irq_of_parse_and_map(dev_node, 0);
	if (ndev->irq == NO_IRQ) {
		printk(KERN_ERR "TXEmac %d: irq not found.\n", Config.DeviceId);
		rc = -ENODEV;
		goto error;
	}
	if (of_address_to_resource(dev_node, 0, &r_mem)) {
...
/*    Config.Dre             = pdata->dre; */
	...
	/* Set the MAC address */
	mac_addr = of_get_property(dev_node, "local-mac-address", NULL);
	if (mac_addr == NULL) {
		printk(KERN_ERR
		       "%s: XTemac: Can't find local-mac-address property.\n",
		       dev_node->full_name);
		rc = -ENXIO;
		goto error;
	}
...
static struct of_device_id xilinx_temac_match[] = {
	{
	 .type = "network",
	 .compatible = "xilinx,temac",
	 },
	{},
};

static struct of_platform_driver xtenet_driver = {
	.name = DRIVER_NAME,
	.match_table = xilinx_temac_match,
	.probe = xtenet_probe,
...
static int __init xtenet_init(void)
{
	 ...
	 */
	return of_register_platform_driver(&xtenet_driver);
}

static void __exit xtenet_cleanup(void)
{
	of_unregister_platform_driver(&xtenet_driver);
}
